...
def verify_input(prompt, valid_values, process_function=None):
    ...
    while True:
        ...
        if process_function is not None:
            input_value = process_function(input_value)
        if input_value not in valid_values:
            print("input {} is not valid, try again".format(input_value))
        else:
            print("-"*15)
            return input_value
